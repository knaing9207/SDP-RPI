...
class MedicationDispenser(QWidget):
    ...
    def setupGPIO(self):
        ...
        self.button_pins = [0, 1, 22, 23, 5, 6, 16, 26]  # Example BCM GPIO numbers, adjust as needed
    ...
    def checkButtonPresses(self):
        ...
        for i, pin in enumerate(self.button_pins):
            if not GPIO.input(pin):  # Button is pressed (GPIO pin is low)
                self.buttons[i].setStyleSheet("font-size: 28px; padding: 30px; background-color: red; color: white;")
    ...
    def buttonClicked(self, button_index):
        ...
        print(f"{selected_option} selected")  # For demonstration
